...
const string strMessageMagic = "Rikeza Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
		...
		if (pindexBest->nHeight < YEAR1_BLOCKS)	{
	    nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
    } 
		else if (pindexBest->nHeight < YEAR2_BLOCKS)	{
	    nSubsidy = nCoinAge * COIN_YEAR1_REWARD * 33 / (365 * 33 + 8);
    }  
		else if (pindexBest->nHeight < YEAR3_BLOCKS)	{
	    nSubsidy = nCoinAge * COIN_YEAR2_REWARD * 33 / (365 * 33 + 8);
    } 
		else if (pindexBest->nHeight < YEAR4_BLOCKS)	{
	    nSubsidy = nCoinAge * COIN_YEAR3_REWARD * 33 / (365 * 33 + 8);
    } 
		else if (pindexBest->nHeight < YEAR5_BLOCKS)	{
	    nSubsidy = nCoinAge * COIN_YEAR4_REWARD * 33 / (365 * 33 + 8);
    } 
		else if (pindexBest->nHeight < YEAR6_BLOCKS)	{
	    nSubsidy = nCoinAge * COIN_YEAR5_REWARD * 33 / (365 * 33 + 8);
    } 
		else if (pindexBest->nHeight < YEAR7_BLOCKS)	{
	    nSubsidy = nCoinAge * COIN_YEAR6_REWARD * 33 / (365 * 33 + 8);
    } 
		else if (pindexBest->nHeight < YEAR8_BLOCKS)	{
	    nSubsidy = nCoinAge * COIN_YEAR7_REWARD * 33 / (365 * 33 + 8);
    }
    else {
      nSubsidy = nCoinAge * COIN_YEAR7_REWARD * 33 / (365 * 33 + 8);
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Rikeza", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "1635669854 beginning of a new journey";
        CTransaction txNew;
        txNew.nTime = 1635669854;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1635669854;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 999150;
        
        
        assert(block.hashMerkleRoot == uint256("0xc5a365598035ab5b1a4db719b222cf0867ecd9f0e0fc98f06d49daf48b06d361"));
